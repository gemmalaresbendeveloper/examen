...
function revisar_dni_personal(){
	...
	if (nombre.length>0  && apemat.length>0 && apepat.length>0) {
	}else{
		return swal("Faltan Llenar Datos","","info");
	}

//    const curpPattern = /^[A-Z][AEIOU][A-Z][A-Z][A-Z][0-9]{2}[0-9]{2}[0-9]{2}[HM][A-Z]{2}[0-9A-Z][0-9A-Z][0-9A-Z][0-9A-Z][0-9A-Z]$/;
//    var curpVal = false;

//    const input =curp;
//    if (curpPattern.test(input)) {
...
//    }
// 	if(!curpVal){
// 		return swal("Lo sentimos la CURP no tiene un formato válido","","warning");
// 	}

	$.ajax({
		url:'../controlador/pacientes/controlador_verificar_existencia_dni.php',
		type:'POST',
		data:{
			curp:curp
		}
	...
	.done(function(resp){
		...
		if (data.length<=0) {
			...
			swal("Lo sentimos la cedula Ingresada ya esta siendo utilizado por otro medico","","warning");
		}
	})
}
function Registrar_personal(){
	...
	$.ajax({
		url:'../controlador/pacientes/controlador_registrar_personal.php',
		type:'POST',
		data:{
			nombre:nombre,
			apepat:apepat,		
	...
	.done(function(resp){
		if (resp > 0) {
			 ...
			 .then ( ( value ) =>  { 
				  $("#main-content").load("Pacientes/vista_registrar_personal.php"); 
			});
