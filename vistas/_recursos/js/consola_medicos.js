...
function revisar_dni_personal(){
	...
	var apepat    = $("#txtapellidopaterno").val();
	var apemat    = $("#txtapellidomaterno").val();
	var dni       = $("#txtcedula").val();
	
	if (nombre.length>0  && apemat.length>0 && apepat.length>0 && dni.length==10) {
	}else{
		return swal("Faltan Llenar Datos","","info");
	}
	if (dni.length!=10 ) {
		return swal("La cédula debe ser a 10 dígitos","","info");
	}

	$.ajax({
		url:'../controlador/medicos/controlador_verificar_existencia_dni.php',
		type:'POST',
		data:{
			dni:dni
		}
...
function Registrar_personal(){
	...
	$.ajax({
		url:'../controlador/medicos/controlador_registrar_personal.php',
		type:'POST',
		data:{
			nombre:nombre,
			apepat:apepat,		
	...
	.done(function(resp){
		if (resp > 0) {
			 ...
			 .then ( ( value ) =>  { 
				  $("#main-content").load("Medicos/vista_registrar_personal.php"); 
			});
