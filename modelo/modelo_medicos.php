<?php
	class Modelo_medicos
	{
		...
		// }
		function listar_combo_diagnosticos(){
			$sql = "SELECT * FROM diagnosticos ORDER BY id_diagnostico DESC";
		...
		function Registrar_medico($nombre,$dni){
			$sql = "call PA_REGISTRARMEDICO('$nombre','$dni')";
			if ($resultado = $this->conexion->conexion->query($sql)){
				return 1;
			}
			else{
		...
		function listar_ciudadano($valor, $inicio=FALSE,$limite=FALSE){
 		...
 		function buscar_medico($dni){
			$sql = "SELECT * from medicos where cedula = '$dni'";			
			$arreglo = array();
			if ($consulta = $this->conexion->conexion->query($sql)) {
				while ($consulta_VU = mysqli_fetch_array($consulta)) {
					$arreglo[] = $consulta_VU;
